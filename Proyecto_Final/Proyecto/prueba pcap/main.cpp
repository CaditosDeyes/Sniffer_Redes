...
void codigoError(int codigo){
    ...
    switch(codigo){
        ...
        case 2:
            ...
            break;
        case 3:
            cout << "no se puede llegar al puerto destino o la aplicación destino no esta libre";
            break;
        case 4:
...
int main(int argc, char **argv)
{
	...
	if(argc < 3)
	{
		...
		printf("Enter the interface number (1-%d):",x);
		scanf("%d", &inum);
		...
		for (d=alldevs, x=0; x< inum-1 ;d=d->next, x++);
    ...
    file_1 = fopen("file_1.txt", "w"); // Crea archivo

	/* Read the packets */
	while((res = pcap_next_ex( fp, &header, &pkt_data)) >= 0) // Contador | ra+ w+
	{
	    if(contador<=2){
            if(res == 0)
                /* Timeout elapsed */
            ...
            /* Print the packet */
            for (x=1; (x < header->caplen + 1 ) ; x++)
            {
                printf("%.2x ", pkt_data[x-1]);

                fputc(pkt_data[x-1], file_1); // Guarda caracteres

                if ( (x % LINE_LEN) == 0) printf("\n");
            }
                //Guardar variable pkt_data en un archivo "archivo.txt"
	...
	//Poner su programa principal hasta DNS leyendo "archivo.txt"
	FILE*archivo;
    ...
    if ((archivo = fopen("file_1.txt", "r+")) == NULL){
